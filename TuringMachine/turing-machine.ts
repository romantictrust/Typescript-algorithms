...
export default class TuringMachine {
  ...
  private lastExecutionResult: ExecutionResult | undefined = undefined;
  ...
  public loadWord(word: string): void {
    ...
    this.tapeSubject.next(
      new Event(EventType.TAPE_MOVE, "move", {
  ...
  private readSymbol(): void {
    this.tapeSubject.next(
      new Event(EventType.SYMBOL_READ, "read", {
  ...
  private execute(): void {
    ...
    this.tapeSubject.next(
      new Event(EventType.SYMBOL_READ, "execute", {
        symbol: this.lastExecutionResult.symbol,
      })
    );
  }

  private writeSymbol(executionResult: ExecutionResult | undefined): void {
    if (executionResult) {
      this.tape.writeSymbol(executionResult?.symbol);
      this.tapeSubject.next(
        new Event(EventType.SYMBOL_WRITE, "write", {
          state: this.stateManager.currentState,
          tape: this.tape,
        })
      );
  ...
  private move(executionResult: ExecutionResult | undefined): void {
    if (executionResult) {
  ...
  private handleError(error: Error): void {
    this.subscription.unsubscribe();
    this.tapeSubject.next(
      new Event(EventType.ERROR, "error", { message: error.message })
    );
  }
}
